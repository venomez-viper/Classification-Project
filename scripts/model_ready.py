...
#
# IMPORTANT: Run this only AFTER task1_features.py AND task2_features.py have finished!
# Reads from outputs/features/task1/ and outputs/features/task2/
# Writes to outputs/model_ready/
#
# Usage: python model_ready.py
...
def main():
    ensure_folders()
    print("model ready review starting...\n")
    print("run this only after task1_features.py and task2_features.py have finished\n")
    ...
    for label, df_ref, df_name in [("Task 1", "task1", task1), ("Task 2", "task2", task2)]:
        weak = find_weak_features(df_name)
        if weak:
            print(f"  {label} near-constant features found:")
            for col, pct in weak:
                print(f"    {col}: {pct}% same value -> dropping")
            drop_cols = [col for col, _ in weak]
            if label == "Task 1":
                task1 = task1.drop(columns=drop_cols)
            else:
                task2 = task2.drop(columns=drop_cols)
            print(f"  Dropped {len(drop_cols)} column(s)")
        else:
            print(f"  {label}: no near-constant features. Good.")
    ...
    body = [
        ...
        "",
        "## Next step",
        "- Model-ready files are in outputs/model_ready/ - ready for the phase summary and weekly report.",
    ]
    save_markdown_report(FINDINGS_PATH / "model_ready_summary.md", "Model Ready Summary", body)

    print("\nmodel ready files saved, all done.")
